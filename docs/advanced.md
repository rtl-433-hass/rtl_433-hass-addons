...
_This section is for developers of the companion integration and other discovery consumers; if you are only configuring radios, you can skip it._
...
Move any per-radio tuning into `<id>.conf` files in the add-on config directory, `/addon_configs/<slug>/`. The add-on logs that directory with its `<slug>` filled in each time it starts.
...
_This section is for contributors editing these docs, not for add-on users._
