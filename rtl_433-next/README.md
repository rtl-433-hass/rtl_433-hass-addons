...
Per-radio override files live in `/addon_configs/<slug>/`. The add-on log prints that directory and the exact `<id>.conf` path for each detected radio, with `<slug>` already filled in.
